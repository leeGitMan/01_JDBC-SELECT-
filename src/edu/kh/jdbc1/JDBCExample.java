...
public class JDBCExample {
	public static void main(String[] args){
		...
		try {
			...
			//    		 (패키지명   + 클래스명)
			// ->() 안에 작성된 클래스이 객체를 반환
			// -> 메모리에 객체가 생성되고, JDBC 필요 시 알아서 참조해서 사용한다.
			// --> 생략해도 자동으로 메모리 로드가 진행되지만 아직까지는 명시적으로 작성하는 것을 권장
			
			// 2. 연결 정보를 담은 Connection 생성하기
			...
			// 만약 localhost 와 loop back ip 가 안되면 cmd창에서 ipconfig로 자신의 ip 주소를 입력
			...
			// oracle.jdbc.driver.T4CConnection@971d0d8
			...
			// excuteQuery -> SELECT문 수행 메서드, ReusltSet반환
			// SQL 문이 DB에게 갔을 때, SQL을 실행해달라고 질의하는 부분
			...
			while(rs.next()) {
				...
				// 				첫 번째 컬럼부터 순서대로 한 행씩 이동하여
				// 				다음 행이 있을 경우 true 반환 / 없으면 false반환
				...
				// 현재 행의 "SALARY" 숫자 (정수 값) 컬럼을 얻어옴
				Date hireDate = rs.getDate("HIRE_DATE");
				// -> java.sql.Date
				// -> java.util.Date도 가능 함
			...
			// SQLExceptiojn : DB 관련 최상위 예외 ( == 데이터베이스 관련 모든 예외)
			e.printStackTrace();
		}finally {
